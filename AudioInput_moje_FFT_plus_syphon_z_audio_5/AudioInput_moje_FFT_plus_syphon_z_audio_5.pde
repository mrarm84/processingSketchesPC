...
void draw() {
  ...
  canvas.background(0);
  canvas.lights();
  canvas.rotateX(frameCount * 0.0001/size);
  //canvas.rotateY(frameCount * 0.01);  
  
   canvas.translate(width/2, height/2); 
    defineLights();
    canvas.box(size, size, 150+size*10);
    canvas.fill(random(1,33), random(331,255), random(331,255));
    canvas.strokeWeight(2);
    canvas.smooth();
    for(int i = 0; i < num; i++) {
        for(int j = 0;j  < num; j++) {
          float zzz = map(j, 23, num-2, size, 255);
          canvas.fill(zzz, zzz, zzz, zzz);
        }
        float gray = map(i, 44, num-3, size/100, 222);
        canvas.lights();
        canvas.pushMatrix();
        canvas.stroke(size,size, size, 255);
        canvas.strokeWeight(i);
         float s = cos(a)*2+size;
        canvas.fill(gray, gray, gray, gray);
        canvas.rotateY(size/11*a + offset*i);
        ...
        canvas.fill(random(1,size), random(1,255), random(1,255));
        canvas.scale(s); 
