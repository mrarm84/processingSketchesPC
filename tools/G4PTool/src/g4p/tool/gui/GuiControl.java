...
public class GuiControl implements TFileConstants, TDataConstants {
	...
	public GuiControl(Editor editor, CtrlTabView tabs, CtrlSketchView tree, CtrlPropView props) {
		...
		if(editor != null){
			...
			} catch (NullPointerException e) {  // was IOException
				e.printStackTrace();
			}
		}
	...
	public void codeCapture(){
		...
		Sketch sketch = editor.getSketch();
		// get the currently selected tab so we can return to it
		int currTab = sketch.getCurrentCodeIndex(); 
		/* 
		 * Version 4.4.1
		 * These two lines locates the 'gui' tab index by getting the gui code and
		 * comparing it to the code in each tab for a match 
			SketchCode gui_tab = getTab(sketch, PDE_TAB_PRETTY_NAME); 	// Get the sketch code for the tab "gui"
			int gui_tab_index = sketch.getCodeIndex(gui_tab);
		 * 
		 * Version 4.4.2 
		 * These lines can be replaced by the single line below
		 */
		int gui_tab_index = getTabIndex(sketch, PDE_TAB_PRETTY_NAME);
		
		// We now continue as we did in previous versionsin previous versions
		SketchCode gui_tab = sketch.getCode(gui_tab_index);
